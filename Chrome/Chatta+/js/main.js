...
window.ChattaPlus = {
	...
	Settings: {
		...
		UI: {
			...
			Init: () => {
				var newSetting = (
					...
					var info = $("<span/>")
						.addClass("sch_info")
						.attr("data-text", infoText);
					var checkbox = $("<input>")
						.addClass("toggle-switch")
						.attr("id", checkboxId)
						.attr("type", "checkbox");
				...
				ChattaPlus.Settings.UI.settingsWindowWrapper = $(
					"<div/>"
				).addClass("sch_settingsWindowWrapper");

				// Settings Window
				var sch_settingsWindow = $("<div/>")
					.addClass("sch_settingsWindow")
					.appendTo(ChattaPlus.Settings.UI.settingsWindowWrapper);
				$("<div/>")
					.addClass("sch_logoSmall")
					.appendTo(sch_settingsWindow)
					.attr("data-version", "v" + ChattaPlus._v);
				var selectPanel = (tab_panel) => {
					...
					ChattaPlus.Settings.UI.settingsPanels
						.children("div.sch_panel")
						.each((index, el) => {
							var panel_element = $(el);
							panel_element.removeClass("_selected");
							if (
								tab_panel === panel_element.attr("data-panel")
							) {
				...
				// Settings Tabs
				ChattaPlus.Settings.UI.settingsTabs = $("<div/>")
					.addClass("sch_tabs")
					.append(
						$("<span/>")
							.addClass("sch_tab")
							.attr("data-tab", "dispositivi")
							.attr("data-panel", "devices")
					)
					.append(
						$("<span/>")
							.addClass("sch_tab")
							.attr("data-tab", "altre impostazioni")
							.attr("data-panel", "other")
					)
					.append(
						$("<span/>")
							.addClass("sch_tab")
							.attr("data-tab", "about")
							.attr("data-panel", "about")
					)
					.appendTo(sch_settingsWindow)
					.children("span")
					.each((index, el) => {
				...
				// Panels
				ChattaPlus.Settings.UI.settingsPanels =
					$("<div/>").addClass("sch_panels");
				ChattaPlus.Settings.UI.devices = $("<div/>")
					.addClass("sch_panel _selected")
					.attr("data-panel", "devices");
				ChattaPlus.Settings.UI.other = $("<div/>")
					.addClass("sch_panel")
					.attr("data-panel", "other");
				ChattaPlus.Settings.UI.about = $("<div/>")
					.addClass("sch_panel")
					.attr("data-panel", "about");

				// Devices panel
				var streamSettings = $("<div/>")
					.addClass("settingsContainer streamSettings")
					.attr("data-title", "Streaming");
				var vocalSettings = $("<div/>")
					.addClass("settingsContainer vocalSettings")
					.attr("data-title", "Messaggi Vocali");
				...
				ChattaPlus.Settings.UI.streamWebcamSelector = $("<select/>")
					...
					.on("change", (evt) => {
						ChattaPlus.WebRTC.Publisher.setDevice(
							"videoinput",
							evt.target.value,
				...
				ChattaPlus.Settings.UI.vocalMicrophoneSelector = $("<select/>")
					.append($("<option/>").text("Seleziona microfono.."))
					.on("change", (evt) => {
						ChattaPlus.WebRTC.Vocal.setDevice(
							evt.target.value,
							ChattaPlus.Settings.SaveSettings
						);
					});

				streamWebcamOptionWrapper.append(
					ChattaPlus.Settings.UI.streamWebcamSelector
				);
				streamMicrophoneOptionWrapper.append(
					ChattaPlus.Settings.UI.streamMicrophoneSelector
				);
				vocalMicrophoneOptionWrapper.append(
					ChattaPlus.Settings.UI.vocalMicrophoneSelector
				);
				...
				// Other panel
				var otherSettings = $("<div/>").addClass(
					"settingsContainer otherSettings"
				);

				newSetting(
					"filtro caps:",
					"I messaggi contenenti molte lettere maiuscole, saranno trasformati in minuscolo",
					"sch_anticaps",
					ChattaPlus.Settings.data.capsFilter,
					false,
					(state) => {
				...
				newSetting(
					"Player youtube grande",
					"Aumenta le dimensioni dei video YouTube",
					"sch_bigyoutubeplayer",
					ChattaPlus.Settings.data.bigYoutubePlayer,
					false,
					(state) => {
				...
				newSetting(
					"cancellazione echo: ",
					"Abilita la cancellazione dell'echo (può ridurre la qualità di registrazione)",
					"sch_echoCancellation",
					ChattaPlus.Settings.data.echoCancellation,
					false,
					(state) => {
				...
				newSetting(
					"audio stereo: ",
					"Registra audio stereo (quando disponibile)",
					"sch_stereoAudio",
					ChattaPlus.Settings.data.stereoAudio,
					ChattaPlus.Settings.data.echoCancellation,
					(state) => {
						...
						$("<p>")
							...
							.append(
								$("<a/>")
									...
									.attr(
										"href",
										"https://www.chatta.it/community/404UserNotFound/default.aspx"
									)
									.attr("target", "_blank")
									.attr("rel", "noopener noreferrer")
									.text("@404UserNotFound")
							)
					)
				);
				ChattaPlus.Settings.UI.about.append(
				...
				ChattaPlus.Settings.UI.about.append(
					$("<div/>").append(
						$("<p>").append(
							$("<a/>")
								.addClass("sch_donate paypal")
								.attr(
									"href",
									"https://www.paypal.com/donate/?hosted_button_id=A7PUH7X6WNVGL"
								)
								.attr("target", "_blank")
								.attr("rel", "noopener noreferrer"),
							$("<span/>")
								.addClass("sch_donate btc")
								.on("click", showBTCAddress),
							(ChattaPlus.Settings.BTCAddress = $("<div/>")
								.addClass("btc_address")
								.append(
									$("<span/>")
										.text(
											"1MBjRHQKn2iEQmGcMMznbm3DToteE9Sbpt"
										)
										.attr("data-text", "BTC Address:")
								))
						)
					)
				);
				ChattaPlus.Settings.UI.about.append(
					$("<div/>").append(
						$("<p>").text(
							"Questa estensione è gratuita e sempre lo sarà!"
						),
						$("<p>").text(
							"Tutte le donazioni saranno utilizzate per il mantenimento e l'upgrade del server"
						)
					)
				);
				...
				ChattaPlus.Settings.UI.settingsPanels.appendTo(
					sch_settingsWindow
				);
				sch_settingsWindow.appendTo(
					ChattaPlus.Settings.UI.settingsWindowWrapper
				);
				ChattaPlus.Settings.UI.settingsWindowWrapper
					.hide()
				...
				$("<div>")
					.addClass("chat-settings-item sch_settingsButtonContainer")
					.html(
						'<label class="text" for="chat-settings-ChattaPlus">Impostazioni Chatta+</label>'
					)
					.append(
						$("<span/>")
							.addClass("sch_settingsButton")
							.attr("data-text", "Apri impostazioni")
					)
		...
		LoadSettings: () => {
			...
			ChattaPlus.Settings.data =
				ChattaPlus.Utils.LocalStorage.Read(key) ||
				ChattaPlus.Settings.data;

			if (ChattaPlus.Settings.data.stream.videoDeviceId != null) {
				ChattaPlus.WebRTC.Publisher.constraints.video.deviceId =
					ChattaPlus.Settings.data.stream.videoDeviceId;
			}

			if (ChattaPlus.Settings.data.stream.audioDeviceId != null) {
				ChattaPlus.WebRTC.Publisher.constraints.audio.deviceId =
					ChattaPlus.Settings.data.stream.audioDeviceId;
			}

			if (ChattaPlus.Settings.data.vocal.audioDeviceId != null) {
				ChattaPlus.WebRTC.Vocal.constraints.audio.deviceId =
					ChattaPlus.Settings.data.vocal.audioDeviceId;
			}
		},
		SaveSettings: () => {
			var key = ChattaPlus._appPrefix + "_settings";
		...
		startWebRTC: () => {
			if (navigator.mediaDevices.getUserMedia) {
				navigator.mediaDevices
					...
					.then(ChattaPlus.Settings.onGetUserMediaSuccess)
					.catch(ChattaPlus.Settings.onGetMediaError);
				ChattaPlus.WebRTC.Publisher.newAPI = true;
			} else if (navigator.getUserMedia) {
				navigator.getUserMedia(
					{ video: true, audio: true },
					ChattaPlus.Settings.onGetUserMediaSuccess,
					ChattaPlus.Settings.onGetMediaError
				);
			} else {
				alert(
					"Il tuo browser non supporta WebRTC, oppure è disabilitato."
				);
			}
		},
		onGetUserMediaSuccess: (stream) => {
			ChattaPlus.Settings.getMediaDevices().then(
				ChattaPlus.Settings.onGetMediaDevices
			);
		},
		onGetMediaError: (e) => {
		...
		onGetMediaDevices: (mediaDevices) => {
			...
			mediaDevices.forEach((mediaDevice) => {
				...
				if (mediaDevice.kind === "videoinput") {
					...
					ChattaPlus.Settings.UI.streamMicrophoneSelector.append(
						option
					);
				}
			});
			mediaDevices.forEach((mediaDevice) => {
				...
				if (mediaDevice.kind === "audioinput") {
					...
					ChattaPlus.Settings.UI.vocalMicrophoneSelector.append(
						option
					);
				}
			});
	...
	Init: () => {
		if(window.location.search.includes("room=")){
			...
			ChattaPlus.overrideMethods();
		}
	},
	overrideMethods: () => {
		if(window.openWebcam){
		}
	},
	overrideChatMethods: () => {
		...
		Chat.textToHtml = function (text) {
			...
			var videoWidth = ChattaPlus.Settings.data.bigYoutubePlayer
				? ChattaPlus.Settings.data.bigYoutubePlayerWidth
				: Chat.mediaMaxWidth;
			...
			if (yt_shorts) {
				videoWidth = ChattaPlus.Settings.data.bigYoutubePlayer
					? ChattaPlus.Settings.data.bigYoutubePlayerWidth
					: Chat.mediaMaxWidth;
			...
			html = html.replace(
				/(\[video\=([a-zA-Z0-9\-\_]*)\])/im,
				'<div class="youtubeplayer"><iframe title="Video (YouTube)" src="//www.youtube.com/embed/$2?rel=0" frameborder="0" width="' +
					videoWidth +
					'" height="' +
					videoHeight +
			...
			// Vocals
			var vocal_tag_regexp = /(\[vocal\:([a-zA-Z0-9\-]*)\])/im;
			var vocal_url_regexp = /(https\:\/\/vocal.schiatta.it\/([a-zA-Z0-9\-]*))/im;
			var vocal_matches_tag = html.match(vocal_tag_regexp);
			var vocal_matches_url = html.match(vocal_url_regexp);

			var vocal_matches = vocal_matches_tag ? vocal_matches_tag : (vocal_matches_url ? vocal_matches_url : false);
			var vocal_regexp = vocal_matches_tag ? vocal_tag_regexp : (vocal_matches_url ? vocal_url_regexp : false);

			if (vocal_matches) {
				if (vocal_matches.length > 0) {
					if (vocal_matches[2].length > 0) {
						var id = vocal_matches[2];
						...
						html = html.replace(vocal_regexp, ChattaPlus.Utils.createVocalPlayer(id));
					}
				}
			}
			...
			if (tiktok_matches) {
				var tiktok_html =
					ChattaPlus.Utils.TikTok.buildPreview(tiktok_matches);
				if (tiktok_html) {
					doNotParseEmoji = true;
			...
			html = html.replace(
				/(^|\s)+(?:@)([a-zA-Z0-9\.]{3,20})($|\s)+/gim,
				' <a href="' +
					BaseLibrary.getUserProfileUrl("$2") +
					'" target="_blank">$2</a> '
			);
		...
		Chat.getCurrentUserStreamSettings = function () {
			return !Chat.isCurrentUserStreamActive
				? {}
				: {
						isVideoEnabled: ChattaPlus.WebRTC.Publisher.videoEnabled,
						isAudioEnabled: ChattaPlus.WebRTC.Publisher.audioEnabled
				  };
		};
			...
			var openStream = function (enableVideoRequestsModeration) {
				Chat.invokeApi(
					"open-stream.ashx",
					{
						...
						enableVideoRequestsModeration: enableVideoRequestsModeration ? "1" : ""
					},
					function (data) {
						var streamId = data;
		...
		Chat.openUserStream = function (userId) {
			...
			var $user = Chat.$usersPanel.find(
				".user[data-user-id='" + userId + "']"
			);
			var nickname = $user.attr("data-nickname");
			$user.find("div.open-user-stream-button").addClass("active");
			var $userStreamWindow = $("<div>")
				.addClass("chat-overlay-window chat-stream-viewer-window")
				.attr("data-user-id", userId)
				.appendTo($("body"));
			$("<div>")
				.addClass("title")
				.html(nickname)
				.appendTo($userStreamWindow);
			$("<div>")
				.addClass("window-control-button close")
				.appendTo($userStreamWindow)
				.attr("title", "Chiudi")
				.tooltipster({ position: "top" })
				.on("click", function () {
					Chat.closeUserStream(userId);
				});
			$("<div>")
				.addClass("window-control-button minimize")
				.appendTo($userStreamWindow)
				.attr("title", "Visualizzazione piccola")
				.tooltipster({ position: "top" })
				.on("click", function () {
					$userStreamWindow.addClass("minimized");
				});
			$("<div>")
				.addClass("window-control-button maximize")
				.appendTo($userStreamWindow)
				.attr("title", "Visualizzazione grande")
				.tooltipster({ position: "top" })
				.on("click", function () {
					$userStreamWindow.removeClass("minimized");
				});
			...
			Chat.invokeApi(
				"get-stream.ashx",
				{
					id: Chat.currentChatRoom.id,
					publisherId: userId,
					onlineUserStatus: Chatta.CurrentUser.OnlineUserStatus
				},
				function (data) {
					if (!data.id) {
						$playerContainer
							.empty()
							.html("In attesa del consenso...");
						return;
					}
					streamId = data.id;
		...
		Chat._createUserStreamPlayer = function ($userStreamWindow, streamId) {
			...
			$(
				"<video autoplay controls playsinline width='100%' height='100%'></video>"
			)
				.addClass("player")
			...
			ChattaPlus.WebRTC.Player.OpenStream(
				streamId,
				errorHandler,
				onPlayPeerConnected,
		...
		Chat.closeUserStream = function (userId) {
			var $stream = $("body").find(
				"div.chat-stream-viewer-window[data-user-id='" + userId + "']"
			);
			if ($stream.hasClass("preclip-active"))
				// wait until video adv ends
				return false;
			var index = -1;
			for (var i = 0; i < Chat.openedUserStreams.length; i++) {
				if (Chat.openedUserStreams[i] === userId) {
			...
			Chat.$usersPanel
				.find(
					".user[data-user-id='" +
						userId +
						"'] div.open-user-stream-button"
				)
				.removeClass("active");
			$stream.remove();
			Chat.invokeApi(
				"release-stream.ashx",
				{ id: Chat.currentChatRoom.id, publisherId: userId },
				null,
				null,
				null
			); // no feedback on success or error
		};
	},
	WebRTC: {
		sdpURL: "wss:/stream.schiatta.it/webrtc-session.json",
		...
		Publisher: {
			...
			UI: {
				...
				Init: () => {
					var videoElement,
						...
						closeStreamButton = null;
					var muted = true;

					// Publisher window
					ChattaPlus.WebRTC.Publisher.UI.videoContainer = $("<div/>")
						.addClass("player-container sch_video-pub")
					...
					ChattaPlus.WebRTC.Publisher.UI.toggleVideoButton = $(
						"<div/>"
					)
					...
					ChattaPlus.WebRTC.Publisher.UI.toggleAudioButton = $("<div/>")
						...
						.on("click", (evt) => {
							ChattaPlus.WebRTC.Publisher.audioEnabled =
								!ChattaPlus.WebRTC.Publisher.audioEnabled;
							ChattaPlus.WebRTC.Publisher.setAudioState(
								ChattaPlus.WebRTC.Publisher.audioEnabled
							);
							$(evt.target).toggleClass("_disabled");
						});
					...
					videoOverlay = $("<div/>")
						...
						.append(
							ChattaPlus.WebRTC.Publisher.UI.toggleAudioButton
						)
						.hide();

					videoElement = $("<video width='100%' height='100%'/>")
						.addClass("player")
					...
					ChattaPlus.WebRTC.Publisher.UI.draggablePublisherWindow =
						$("<div/>").addClass(
							"chat-overlay-window chat-stream-viewer-window owner"
						)
						//.append(videoOverlay)
						//.append(ChattaPlus.WebRTC.Publisher.UI.liveIndicator)
					...
					$("<div>")
						...
						.html("La mia webcam")
						.appendTo(
							ChattaPlus.WebRTC.Publisher.UI
								.draggablePublisherWindow
						);

					$("<div>")
						.addClass("window-control-button minimize")
						.appendTo(ChattaPlus.WebRTC.Publisher.UI
							.draggablePublisherWindow)
						.attr("title","Visualizzazione piccola"
						)
						.tooltipster({ position: "top" })
						.on("click", function () {
							ChattaPlus.WebRTC.Publisher.UI.draggablePublisherWindow.addClass(
								"minimized"
							);
						});
					$("<div>")
						...
						.tooltipster({ position: "top" })
						.on("click", function () {
							ChattaPlus.WebRTC.Publisher.UI.draggablePublisherWindow.removeClass(
								"minimized"
							);
						});
					$("<div>")
						...
						.attr("title", "Disattiva la tua webcam")
						.tooltipster({ position: "top" })
						.on("click", Chat.closeCurrentUserStream);
					ChattaPlus.WebRTC.Publisher.UI.draggablePublisherWindow.drags(
						{ handle: "div.title" }
					);
					ChattaPlus.WebRTC.Publisher.UI.videoContainer.appendTo(
						ChattaPlus.WebRTC.Publisher.UI
							.draggablePublisherWindow
					);
					ChattaPlus.WebRTC.Publisher.UI.draggablePublisherWindow
			...
			startWebRTC: () => {
				...
				ChattaPlus.WebRTC.Publisher.localVideo =
					document.getElementById(
						"vpe_" + Chatta.CurrentUser.Nickname
					);
				...
				if (navigator.mediaDevices.getUserMedia) {
					navigator.mediaDevices
						.getUserMedia(ChattaPlus.WebRTC.Publisher.constraints)
						.then(ChattaPlus.WebRTC.Publisher.onGetUserMediaSuccess)
						.catch(ChattaPlus.WebRTC.Publisher.onGetMediaError);
					ChattaPlus.WebRTC.Publisher.newAPI = true;
				} else if (navigator.getUserMedia) {
					navigator.getUserMedia(
						ChattaPlus.WebRTC.Publisher.constraints,
						ChattaPlus.WebRTC.Publisher.onGetUserMediaSuccess,
						ChattaPlus.WebRTC.Publisher.onGetMediaError
					);
				} else {
					alert(
						"Il tuo browser non supporta WebRTC, oppure è disabilitato."
				...
				console.log(
					"ChattaPlus.InitWebRTC: Complete" +
						(ChattaPlus.WebRTC.Publisher.newAPI ? " (NewAPI)" : "")
				);
			},
			stopWebRTC: () => {
				...
				Chat.$toggleCurrentUserStreamButton.removeClass("active");
				if(ChattaPlus.WebRTC.Publisher.UI.videoContainer){
			...
			onGetUserMediaSuccess: (stream) => {
				...
				ChattaPlus.WebRTC.Publisher.audioEnabled = true;
				ChattaPlus.WebRTC.Publisher.videoEnabled = true;
			...
			stopMediaAccess: () => {
				...
				if (ChattaPlus.WebRTC.Publisher.localStream != null) {
					try {
						ChattaPlus.WebRTC.Publisher.localStream
							.getAudioTracks()
			...
			onConnectionStateChange: (evt) => {
				if (evt.target != null && evt.target.connectionState != null) {
					...
					switch(ChattaPlus.WebRTC.Publisher.connectionState){
						case "connecting":
							Chat.isCurrentUserStreamActive = false;
							ChattaPlus.WebRTC.Publisher.UI.liveIndicator.hide();
							break;
						case "connected":
							WowzaPeerConnectionPublish.setVideoBandwith( ChattaPlus.WebRTC.Publisher.media.videoBitrate );
						...
						case "disconnected":
							Chat.isCurrentUserStreamActive = false;
							ChattaPlus.WebRTC.Publisher.UI.liveIndicator.hide();
							ChattaPlus.WebRTC.Publisher.UI.videoContainer.html('Si è verificato un errore');
							break;
					}
				}
			},
			...
			ToggleStream: (streamId) => {
				if (Chat.isCurrentUserStreamActive) {
					...
					if (streamId) {
						...
						console.log(
							"ChattaPlus.ToggleStream: Failed - No streamId"
						);
					}
				}
			},
			setTrackState: (trackKind, state) => {
				if (
					ChattaPlus.WebRTC.Publisher.localStream != null &&
					ChattaPlus.WebRTC.Publisher.localStream.getTracks != null
				) {
					ChattaPlus.WebRTC.Publisher.localStream
						.getTracks()
						.map((track) => {
			...
			setDevice: (kind, deviceId, callback) => {
				if (
					...
					ChattaPlus.WebRTC.Publisher.localStream
						...
						.forEach(function (track) {
							track.stop();
							ChattaPlus.WebRTC.Publisher.localStream.removeTrack(
								track
							);
						});
				}
			...
			replaceTracks: (newStream) => {
				if (
					ChattaPlus.WebRTC.Publisher.localVideo != null &&
					ChattaPlus.WebRTC.Publisher.localStream
				) {
					ChattaPlus.WebRTC.Publisher.localVideo.pause();
					if (
						typeof ChattaPlus.WebRTC.Publisher.localVideo
							.srcObject === "object"
					) {
						ChattaPlus.WebRTC.Publisher.localVideo.srcObject = null;
					} else {
					...
					if (
						...
						ChattaPlus.WebRTC.Publisher.localVideo.srcObject =
							newStream;
					} else {
						ChattaPlus.WebRTC.Publisher.localVideo.src =
							window.URL.createObjectURL(newStream);
					}

					ChattaPlus.WebRTC.Publisher.localVideo.onloadedmetadata =
						function (e) {
		...
		Player: {
			...
			stop: (stream) => {
				...
				ChattaPlus.WebRTC.Player.streams =
					ChattaPlus.WebRTC.Player.streams.filter(function (item) {
						return item !== stream;
					});
			...
			errorHandler: (error) => {
				...
				if (error.message == null) {
					if (error.target != null) {
						console.log(
							"typeof error.target: " + typeof error.target
						);
					}
				}
			},
			OpenStream: (streamId) => {
				var stream = {
					connectionState: "stopped",
					playerVideoElement: document.getElementById(
						"video_" + streamId
					),
					streamInfo: {
				...
				stream.onConnectionStateChange = (evt) => {
					if (
						evt.target != null &&
						evt.target.connectionState != null
					) {
		...
		Vocal: {
			...
			UI: {
				Init: () => {
					...
					ChattaPlus.WebRTC.Vocal.UI.audioPreview =
						new window.Audio();
					...
					ChattaPlus.WebRTC.Vocal.UI.toggleVocalRecorderButton = $(
						"<div/>"
					)
					...
					ChattaPlus.WebRTC.Vocal.UI.vocalContainer.append(
						ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton
					);
					ChattaPlus.WebRTC.Vocal.UI.vocalContainer.append(
						ChattaPlus.WebRTC.Vocal.UI.vocalTimer
					);
					ChattaPlus.WebRTC.Vocal.UI.vocalContainer.append(
						ChattaPlus.WebRTC.Vocal.UI.vocalPreviewButton
					);
					ChattaPlus.WebRTC.Vocal.UI.vocalContainer.append(
						ChattaPlus.WebRTC.Vocal.UI.vocalSendButton
					);

					$(".chat-composer").append(
						ChattaPlus.WebRTC.Vocal.UI.toggleVocalRecorderButton
					);
					$(".chat-composer").append(
						ChattaPlus.WebRTC.Vocal.UI.vocalContainer
					);
					console.log("Vocal Ui initialized");
				},
				ToggleRecorder: () => {
					if (ChattaPlus.WebRTC.Vocal.localStream != null) {
						ChattaPlus.WebRTC.Vocal.UI.vocalContainer.slideToggle(
							250
						);
						ChattaPlus.WebRTC.Vocal.stopWebRTC();
					} else {
			...
			onGetUserMediaSuccess: (stream) => {
				...
				ChattaPlus.WebRTC.Vocal.mediaRecorder = new MediaRecorder(
					stream,
					options
				);
				ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.removeClass(
					"_disabled"
				);
			},
			onGetUserMediaError: () => {
			...
			startWebRTC: () => {
				...
				ChattaPlus.WebRTC.Vocal.constraints.audio.echoCancellation =
					ChattaPlus.Settings.data.echoCancellation;
				ChattaPlus.WebRTC.Vocal.constraints.audio.noiseSuppression =
					ChattaPlus.Settings.data.stereoAudio ? false : true;

				if (navigator.mediaDevices.getUserMedia) {
					navigator.mediaDevices
						.getUserMedia(ChattaPlus.WebRTC.Vocal.constraints)
						.then(ChattaPlus.WebRTC.Vocal.onGetUserMediaSuccess)
						.catch(ChattaPlus.WebRTC.Vocal.onGetMediaError);
					ChattaPlus.WebRTC.Vocal.newAPI = true;
				} else if (navigator.getUserMedia) {
					navigator.getUserMedia(
						ChattaPlus.WebRTC.Vocal.constraints,
						ChattaPlus.WebRTC.Vocal.onGetUserMediaSuccess,
						ChattaPlus.WebRTC.Vocal.onGetMediaError
					);
				} else {
					alert(
						"Il tuo browser non supporta WebRTC, oppure è disabilitato."
			...
			stopMediaAccess: () => {
				if (ChattaPlus.WebRTC.Vocal.localStream != null) {
					try {
						ChattaPlus.WebRTC.Vocal.localStream
							.getAudioTracks()
							.forEach(function (track) {
								track.stop();
							});
			...
			Reset: () => {
				clearInterval(
					ChattaPlus.WebRTC.Vocal.localAudioPreviewInterval
				);
				...
				ChattaPlus.WebRTC.Vocal.UI.vocalTimer.attr(
					"data-time",
					"00:00"
				);
				ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.removeClass(
				...
				ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.attr(
					"title",
					"Registra"
				);
				ChattaPlus.WebRTC.Vocal.UI.vocalPreviewButton.removeClass(
					"_playing"
				);
				ChattaPlus.WebRTC.Vocal.UI.vocalPreviewButton.addClass(
					"_disabled"
				);
				ChattaPlus.WebRTC.Vocal.UI.vocalSendButton.addClass(
					"_disabled"
				);
			...
			Record: () => {
				if (
					ChattaPlus.WebRTC.Vocal.mediaRecorder &&
					!ChattaPlus.WebRTC.Vocal.isRecording
				) {
					ChattaPlus.WebRTC.Vocal.UI.vocalTimer.addClass(
						"_recording"
					);
					ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.addClass(
						"_recording"
					);
					ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.attr(
						"title",
					...
					setTimeout(() => {
						ChattaPlus.WebRTC.Vocal.mediaRecorder.ondataavailable =
							ChattaPlus.WebRTC.Vocal.handleDataAvailable;
						...
						ChattaPlus.WebRTC.Vocal.recordInterval = setInterval(
							() => {
								...
								ChattaPlus.WebRTC.Vocal.UI.vocalTimer.attr(
									"data-time",
									"00:" +
										(ChattaPlus.WebRTC.Vocal.recordTime > 9
											? "" +
											  ChattaPlus.WebRTC.Vocal.recordTime
											: "0" +
											  ChattaPlus.WebRTC.Vocal
													.recordTime)
								);
								if (
									ChattaPlus.WebRTC.Vocal.recordTime >=
									ChattaPlus.Settings.data.maxVocalRecordTime
								) {
									console.log(
										"Max record time reached, stopping"
									);
									ChattaPlus.WebRTC.Vocal.Stop();
								}
							},
			...
			Stop: () => {
				if (ChattaPlus.WebRTC.Vocal.isRecording) {
					ChattaPlus.WebRTC.Vocal.UI.vocalTimer.removeClass(
						"_recording"
					);
					ChattaPlus.WebRTC.Vocal.UI.toggleRecordButton.removeClass(
						"_recording"
					...
					ChattaPlus.WebRTC.Vocal.UI.vocalSendButton.removeClass(
						"_disabled"
					);
					...
					setTimeout(() => {
						...
						ChattaPlus.WebRTC.Vocal.createLocalPreviewPlayer(
							ChattaPlus.WebRTC.Vocal.blob
						);
					}, 500);
				}
			},
			createLocalPreviewPlayer: (blob) => {
				...
				ChattaPlus.WebRTC.Vocal.localAudioPreview.addEventListener(
					"play",
					() => {
						ChattaPlus.WebRTC.Vocal.UI.vocalPreviewButton.addClass(
							"_playing"
				...
				ChattaPlus.WebRTC.Vocal.UI.vocalPreviewButton
					...
					.on("click", () => {
						if (ChattaPlus.WebRTC.Vocal.localAudioPreview != null) {
							if (
								ChattaPlus.WebRTC.Vocal.localAudioPreview
									.paused ||
								ChattaPlus.WebRTC.Vocal.localAudioPreview.ended
							) {
								ChattaPlus.WebRTC.Vocal.localAudioPreviewInterval =
									setInterval(() => {
										ChattaPlus.WebRTC.Vocal.UI.vocalTimer.attr(
											"data-time",
											ChattaPlus.Utils.timeFormatter(
												ChattaPlus.WebRTC.Vocal
													.localAudioPreview
													.currentTime
											)
				...
				ChattaPlus.WebRTC.Vocal.UI.vocalSendButton
					.removeClass("_disabled")
					.on("click", () => {
						ChattaPlus.WebRTC.Vocal.UI.vocalSendButton.addClass("_uploading _disabled");
			...
			renderizeVocalMessage: (id, recipient) => {
				var sent = new Date();
				var currentUserInRoom = Chat.getUserInRoom(
					Chatta.CurrentUser.UserId
				);
				var currentUserType = currentUserInRoom
					? Chat.getUserType(currentUserInRoom)
					: Chat.UserType.Member;
				var message = {
					from: {
						id: Chatta.CurrentUser.UserId,
						nickname: Chatta.CurrentUser.Nickname,
						avatarId: Chat.userAvatarId,
						gender: Chatta.CurrentUser.Gender,
						userType: currentUserType
					},
					sent: "oggi " + sent.getHours() + ":" + sent.getMinutes(),
					timestamp: null,
					to: recipient,
					text: "https://vocal.schiatta.it/" + id,
			...
			sendVocalMessageInChat: (id) => {
				...
				var directMessageRecipient =
					ChattaPlus.Utils.getDirectMessageRecipient(true);

				if (directMessageRecipient != null) {
					data["toUserId"] = directMessageRecipient.id;
					$(".chat-composer textarea").val(
						"@" + directMessageRecipient.nickname + " "
					);
				}

				if (
					Chat.enableWebSocket &&
					Chat.webSocket &&
					Chat.webSocketIsOpen
				) {
				...
				ChattaPlus.WebRTC.Vocal.renderizeVocalMessage(
					id,
	...
	Utils: {
		...
		audioFileExists: (id, callback) => {
			...
			xhr.open("GET", "https://vocal.schiatta.it/" + id);
			xhr.setRequestHeader("chattaplus", true);
			xhr.send();
		},
		createVocalPlayer: (id) => {
			var html = `
			<div class="sch_vocalPlayerContainer" id="sch_ws_` + id + `"><div class="sch_vocalPlayer"></div>
			<div class="sch_vocalPlayerTogglePlay"></div>
			<div class="sch_vocalPlayerTime" data-time="00:00"></div>
			<div class="clear-fix"></div>
			<script type="text/javascript">
			ChattaPlus.Utils.audioFileExists('` + id + `', (exist) => {

				if(exist){
					var vocal_container = $("#sch_ws_` + id  + `");
					var ws = WaveSurfer.create({
						container: vocal_container.find(".sch_vocalPlayer")[0],
						waveColor: "#48cae4",
						progressColor: "#0096c7",
						height:30,
						barWidth:3
					});
					ws.load("https://vocal.schiatta.it/` + id + `");
					ws.on("ready", () => {
						ChattaPlus.Utils.onVocalLoad(ws, vocal_container);
					});
					ws.on("finish", () => {
						vocal_container.find(".sch_vocalPlayerTogglePlay").removeClass("_playing");
						vocal_container.find(".sch_vocalPlayerTime").attr("data-time", ChattaPlus.Utils.timeFormatter(ws.getCurrentTime()));
					});
				}else{
					console.log("Vocal id [` + id + `] does not exists");
				}
			});`;
			return html;
		},
		onVocalLoad: (ws, container) => {
			container.find(".sch_vocalPlayerTogglePlay").on("click",() => {
				if(ws.isPlaying()){
					ws.pause();
					container.find(".sch_vocalPlayerTogglePlay").removeClass("_playing");
				} else {
					ws.play();
					container.find(".sch_vocalPlayerTogglePlay").addClass("_playing");
				}
				container.find(".sch_vocalPlayerTime").attr("data-time", ChattaPlus.Utils.timeFormatter(ws.getCurrentTime()));
					ws.on("audioprocess", () => {
						container.find(".sch_vocalPlayerTime").attr("data-time", ChattaPlus.Utils.timeFormatter(ws.getCurrentTime()));
					});
				});
		...
		TikTok: {
			...
			buildPreview: (media) => {
				...
				var preview =
					`<span class="button-show-media tiktok" onclick="(()=>{var self=$(this); var embed=self.parent().find('blockquote'); self.html((embed.is(':visible')?'visualizza TikTok':'nascondi TikTok'));embed.slideToggle();})();"> ` +
					label +
					`</span>`;
